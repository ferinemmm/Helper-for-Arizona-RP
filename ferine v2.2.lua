...
script_description("помощник, за помощью писать в ТГ yabublik00") -- опиcание скрипта
script_version('v.2')
...
function main()
	...
	wait(0)
		...
		if isKeyDown (VK_CONTROL) and isKeyJustPressed (VK_S) then -- отчет /smi
		...
		if isKeyDown (VK_CONTROL) and isKeyJustPressed (VK_K) then -- отчет /members
		...
		if isKeyDown (VK_CONTROL) and isKeyJustPressed (VK_T) then -- отчет /members
